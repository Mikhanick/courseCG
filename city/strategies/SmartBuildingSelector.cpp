...
GraphicObject SmartBuildingSelector::select(const QRectF& availableArea) {
    ...
    float availableWidth = std::max(10.0f, static_cast<float>(availableArea.width()) - 2 * margin);
    float availableDepth = std::max(10.0f, static_cast<float>(availableArea.height()) - 2 * margin);
